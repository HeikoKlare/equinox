...
public class EclipseStarterConfigurationAreaTest {
	private static final String WITH_COMPATIBILITY_FRAGMENT_TAG = "withCompatibilityFragment";
	...
	private CustomSessionConfiguration createSessionConfiguration() {
		this.sessionConfiguration = SessionTestExtension.createCustomConfiguration();
		sessionConfiguration.setConfigurationDirectory(configurationDirectory);
	...
	public void setupSession(TestInfo testInfo) {
		if (testInfo.getTags().contains(WITH_COMPATIBILITY_FRAGMENT_TAG)) {
			sessionConfiguration.addBundle(Platform.getBundle("org.eclipse.osgi.compatibility.state"));
		}
	}

	@Test
	@Tag(WITH_COMPATIBILITY_FRAGMENT_TAG)
	@Order(1)
